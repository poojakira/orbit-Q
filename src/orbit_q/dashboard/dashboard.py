...
def get_mock_telemetry(n: int = 100) -> pd.DataFrame:
    ...
    data = {
        "timestamp": t,
        "temperature_C": np.where(anomaly_mask, np.random.uniform(85, 120, n), np.random.normal(25, 3, n)),
        "voltage_V": np.where(anomaly_mask, np.random.uniform(2.5, 3.5, n), np.random.normal(5.0, 0.1, n)),
...
if "temperature_C" in df.columns:
    with tab1:
        chart_data = (
            df.set_index("timestamp")[["temperature_C"]] if "timestamp" in df.columns else df[["temperature_C"]]
        )
        st.line_chart(chart_data, height=200, use_container_width=True)
        if anomaly_count > 0:
            st.markdown(
                '<div class="anomaly-alert">⚠️ <b>Thermal anomaly detected!</b> Temperature spike on one or more faces.</div>',
                unsafe_allow_html=True,
            )
...
if "signal_strength_dBm" in df.columns:
    with tab3:
        chart_data = (
            df.set_index("timestamp")[["signal_strength_dBm"]]
            if "timestamp" in df.columns
            else df[["signal_strength_dBm"]]
        )
        st.line_chart(chart_data, height=200, use_container_width=True)
...
if anomaly_count > 0:
    ...
    st.markdown(
        '<div class="nominal-chip">✅ All telemetry channels nominal — no anomalies detected.</div>',
        unsafe_allow_html=True,
    )
...
if run_playbook:
    with st.spinner("Executing alert playbook..."):
        ...
        if "Slack" in alert_channel:
            ...
            if webhook:
                try:
                    requests.post(
                        webhook,
                        json={
                            "text": f"🛰️ [ORBIT-Q DRILL] {anomaly_count} anomalies detected - operator review required."
                        },
                        timeout=5,
                    )
            ...
            st.info(
                "📟 **[SIM]** PagerDuty P1 incident would be created — configure PAGERDUTY_ROUTING_KEY for live use."
            )
